...
server <- function(input, output, session) { 
  ...
  # Analysis with bar plot
  output$plot_result <- renderPlot({
    req(result_df$data)
    # call your helper function, passing the VALUE of the reactive
    barplot_results <- analyse_barplot(
      data = result_df$data, # Not result_df$data() unless it's a reactive expression
      var1 = facet_var(),
      var2 = x_var(),
      measure_col = VALUE(),
